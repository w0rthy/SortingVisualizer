...
namespace {
	struct _ : public Visualizer {
		_() {
			name = "Galaxy Spiral Visualizer";
			shader = &shader_basic_3d;
		}

		void init() {
			usePerspective(35.f, 1.f, 0.1f, 100.f);
		}
		...
		mat4 cam_mat = getCamMat(vec3(0.f, 0.f, -4.f));
		float cam_angle = 0.f;
		...
		void drawElement(VisualizerListInfo* inf, List<int>* l, int depthInd, ListElement<int>* e, int pos, int depth, bool marked) {
			...
			vec3 p = mutate*rotate(vec3(0.f, 0.f, 0.2f + dist * 0.6f), vec3(90.f - pos * angstepP, pos * angstepY, 0.f));
...
Visualizer* visualizer_galaxy_spiral = &::_;
